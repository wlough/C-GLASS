...
system_parameters parse_system_params(YAML::Node &node) {
  ...
  for (auto it=node.begin(); it!= node.end(); ++it) {
    ...
    } else if (param_name.compare("draw_centroid")==0) {
    params.draw_centroid = it->second.as<bool>();
    } else if (param_name.compare("draw_mindist")==0) {
    params.draw_mindist = it->second.as<bool>();
    } else if (param_name.compare("enable_flipping")==0) {
    ...
    } else if (param_name.compare("membrane_tether_repulsive_singularity")==0) {
    ...
    } else if (param_name.compare("membrane_tether_attractive_singularity")==0) {
...
species_base_parameters *parse_species_params(std::string sid,
  ...
  if (false) {
    ...
    for (auto jt = subnode.begin(); jt != subnode.end(); ++jt) {
      ...
      params.stationary_until = jt->second.as<int>();
      } else if (param_name.compare("n_attached")==0) {
      params.n_attached = jt->second.as<double>();
  ...
  } else if (sid.compare("membrane") == 0) {
    membrane_parameters params;
    parse_species_base_params(params, node);
    for (auto jt = subnode.begin(); jt != subnode.end(); ++jt) {
      std::string param_name = jt->first.as<std::string>();
      if (false) {
      ...
      params.stationary_until = jt->second.as<int>();
      } else if (param_name.compare("bending_modulus")==0) {
      params.bending_modulus = jt->second.as<double>();
      } else if (param_name.compare("spontaneous_curvature")==0) {
      params.spontaneous_curvature = jt->second.as<double>();
      } else if (param_name.compare("splay_modulus")==0) {
      params.splay_modulus = jt->second.as<double>();
      } else if (param_name.compare("dimensionless_tether_repulsive_singularity")==0) {
      params.dimensionless_tether_repulsive_singularity = jt->second.as<double>();
      } else if (param_name.compare("dimensionless_tether_repulsive_onset")==0) {
      params.dimensionless_tether_repulsive_onset = jt->second.as<double>();
      } else if (param_name.compare("dimensionless_tether_attractive_onset")==0) {
      params.dimensionless_tether_attractive_onset = jt->second.as<double>();
      } else if (param_name.compare("dimensionless_tether_attractive_singularity")==0) {
      params.dimensionless_tether_attractive_singularity = jt->second.as<double>();
      } else if (param_name.compare("tether_stiffness")==0) {
      params.tether_stiffness = jt->second.as<double>();
      } else if (param_name.compare("fix_target_face_area")==0) {
      params.fix_target_face_area = jt->second.as<bool>();
      } else if (param_name.compare("area_stiffness")==0) {
      params.area_stiffness = jt->second.as<double>();
      } else if (param_name.compare("fix_target_volume")==0) {
      params.fix_target_volume = jt->second.as<bool>();
      } else if (param_name.compare("volume_stiffness")==0) {
      params.volume_stiffness = jt->second.as<double>();
      } else if (param_name.compare("node_drag_coefficient")==0) {
      params.node_drag_coefficient = jt->second.as<double>();
      } else if (param_name.compare("kBT")==0) {
      params.kBT = jt->second.as<double>();
      } else if (param_name.compare("enable_flipping")==0) {
      params.enable_flipping = jt->second.as<bool>();
      } else if (param_name.compare("enable_fluctuations")==0) {
      params.enable_fluctuations = jt->second.as<bool>();
      } else if (param_name.compare("dt_flip")==0) {
    ...
    return new membrane_parameters(params);
  } else {
