...
void CrosslinkManager::InitSpecies(sid_label &slab, ParamsParser &parser,
  ...
  if (xlink_species_.back()->GetNInsert() <= 0) {
    ...
    xlink_species_.back()->InitInteractionEnvironment(
        objs_, &obj_size_, tracker_, &update_, &bound_curr_);
...
void CrosslinkManager::UpdateObjsSize() {
  ...
  for (auto it = objs_->begin(); it != objs_->end(); ++it) {
    ...
    case shape::rod:
      ...
      break;
    case shape::sphere:
      if ((*it)->GetNAnchored() == 0)
        obj_size_ += (*it)->GetArea();
      break;
    default:
      break;
    }
...
void CrosslinkManager::CheckForCross() {
  ...
  //are crossing
  for (auto spec_one = xlink_species_.begin(); spec_one != xlink_species_.end();
       ++spec_one) {
    ...
    for (int i = 0; i <= (member_num_ - 1); i++) {
      ...
      if (link_one->IsDoubly() && link_one->ReturnCheckForCross() == true) {
        for (auto spec_two = xlink_species_.begin();
             spec_two != xlink_species_.end(); ++spec_two) {
          ...
          for (int j = 0; j <= (member_num_two_ - 1); j++) {
            ...
            if (link_two->IsDoubly() &&
                link_one->GetOID() != link_two->GetOID()) {
              ...
              if (same_microtubules && ((link_one_s[0] > link_two_s[0] &&
                                         link_one_s[1] < link_two_s[1]) ||
                                        (link_one_s[0] < link_two_s[0] &&
                                         link_one_s[1] > link_two_s[1]))) {
...
//If trace is on print everything in bound_curr
  ...
  for (auto &kv : bound_curr_) {
    Receptor *receptor = kv.first;
    std::pair<std::vector<double>,
              std::vector<std::pair<Anchor *, std::string>>> &receptor_info =
        kv.second;
    Logger::Trace("Receptor id is, %d", receptor->GetOID());
    int events = receptor_info.first.size();
    while (events > 0) {
      events -= 1;
      if (receptor_info.second[events].second == "forward step" ||
          receptor_info.second[events].second == "back step" ||
          receptor_info.second[events].second == "single to double") {
        Logger::Trace("Prop is %f, Anchor is %i, Bind type is %s",
                      receptor_info.first[events],
                      receptor_info.second[events].first->GetOID(),
                      receptor_info.second[events].second.c_str());
      }
    }
  }
  Logger::Trace("\n");
  ...
  for (auto &kv : bound_curr_) {
    ...
    //If two or three anchors want to bind to a receptor choose which one binds
    else if (receptor_info.first.size() == 2 ||
             receptor_info.first.size() == 3) {
      double sum_of_probs = std::accumulate(receptor_info.first.begin(),
                                            receptor_info.first.end(), 0.0);
      Logger::Trace("%i anchors want to bind to site %i with total probability "
                    "%f. If this warning is common, or if probability is much "
                    "greater than 1 reduce time step.",
                    receptor_info.first.size(), receptor->GetOID(),
                    sum_of_probs);
      ...
      for (auto prob = prob_list.begin(); prob != prob_list.end(); ++prob) {
        if (*prob > roll) {
          ...
          Logger::Trace("chose %s, from roll, %f",
                        receptor_info.second[count].second.c_str(), roll);
          roll = 1000000;
        } else {
          roll -= *prob;
          count += 1;
    ...
    // If more than 3 anchors want to bind to a receptor throw error
      ...
      Logger::Error("More than 3 anchors trying to bind to receptor, time step "
                    "too large");
    }
    for (auto it = receptor_info.second.begin();
         it != receptor_info.second.end(); ++it) {
...
void CrosslinkManager::KnockoutBind(Receptor *receptor, int winner) {
  ...
  //Single to double bind
    ...
    Logger::Trace("single to double %i",
                  bound_curr_[receptor].second[winner].first->GetOID());
    Object *ob_pointer =
        bound_curr_[receptor].second[winner].first->GetCrosslinkPointer();
  ...
  //Bind from soultion, bind type is set to species name if binding from solution
    ...
    CrosslinkSpecies *cl_species_ =
        species_map_[bound_curr_[receptor].second[winner].second.c_str()];
    std::pair<CrosslinkSpecies *, Receptor *> cl_rec_pair_;
...
void CrosslinkManager::AddKnockoutCrosslinks() {
  for (auto it = cl_to_add_this_step_.begin(); it != cl_to_add_this_step_.end();
       ++it) {
