...
HalfEdgeSamples vf_samples_to_he_samples(const Samples3d &xyz_coord_V,
  ...
  while (!H_boundary_minus.empty()) {
    int b = h_right_B.size();
    int h_right_b = *H_boundary_minus.begin();
...
he_samples_to_vef_samples(const Samples3d &xyz_coord_V, const Samplesi &h_out_V,
  ...
  for (int f = 0; f < Nf; ++f) {
    ...
    // Insert edges into the set and update V_of_E
    if (setV_of_E.find(edge0) == setV_of_E.end()) {
      V_of_E.row(setV_of_E.size()) << edge0[0], edge0[1];
      setV_of_E.insert(edge0);
    }
    if (setV_of_E.find(edge1) == setV_of_E.end()) {
      V_of_E.row(setV_of_E.size()) << edge1[0], edge1[1];
      setV_of_E.insert(edge1);
    }
    if (setV_of_E.find(edge2) == setV_of_E.end()) {
      V_of_E.row(setV_of_E.size()) << edge2[0], edge2[1];
      setV_of_E.insert(edge2);
    }
  }
  return std::make_tuple(xyz_coord_V, V_of_E, V_of_F);
}
...
HalfEdgeSamples load_he_samples_from_ply(const std::string &filepath,
  ...
  try {
    ...
    const double *xyz_coord_V_buffer =
        reinterpret_cast<const double *>(_xyz_coord_V->buffer.get());
    const int *h_out_V_buffer =
        reinterpret_cast<const int *>(_h_out_V->buffer.get());
    const int *v_origin_H_buffer =
        reinterpret_cast<const int *>(_v_origin_H->buffer.get());
    const int *h_next_H_buffer =
        reinterpret_cast<const int *>(_h_next_H->buffer.get());
    const int *h_twin_H_buffer =
        reinterpret_cast<const int *>(_h_twin_H->buffer.get()); // h_twin
    const int *f_left_H_buffer =
        reinterpret_cast<const int *>(_f_left_H->buffer.get()); // f_left
    const int *h_bound_F_buffer =
        reinterpret_cast<const int *>(_h_bound_F->buffer.get()); // h_bound
    const int *h_right_B_buffer =
        reinterpret_cast<const int *>(_h_right_B->buffer.get()); // h_right
...
VertexEdgeFaceSamples MeshConverter::get_vef_samples() {
